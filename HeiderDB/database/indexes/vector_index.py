...
class VectorIndex:
    ...
    def add_vector(self, id, vector, save_metadata=True):
        """
        ...
        save_metadata: si True, guarda los metadatos tras la inserción (por defecto True para compatibilidad)
        """
    ...
    def batch_save_metadata(self):
        """
        Guarda los metadatos del índice (para uso tras inserciones masivas).
        """
        self.save_metadata()
    
    def remove_vector(self, id, save_metadata=True):
        """
    ...
    def search_knn_with_index(self, query_vector, k=5, clusters_to_check=10):
        """
        Búsqueda KNN usando clustering para filtrar páginas relevantes y acelerar la búsqueda.
        clusters_to_check: número de clusters más cercanos a revisar (por defecto 10).
        """
        import heapq
        if isinstance(query_vector, list):
            query_vector = np.array(query_vector)
        ...
        # Buscar solo en los clusters más cercanos (por defecto 10 más cercanos)
        best_k = []
        visited_ids = set()
        clusters_to_search = [cid for cid, _ in cluster_distances[:min(clusters_to_check, len(cluster_distances))]]
        ...
        if len(best_k) < k:
            for cluster_id, _ in cluster_distances[min(clusters_to_check, len(cluster_distances)):]:
                for vector_id in self.clusters.get(cluster_id, []):
                    if vector_id in visited_ids:
                        continue
                    vector = self.get_vector(vector_id)
