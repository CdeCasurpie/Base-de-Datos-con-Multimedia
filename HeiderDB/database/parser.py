...
def parse_query(query):
    ...
    try:
        ...
        match = re.match(
            create_inverted_index_pattern, query, re.IGNORECASE | re.VERBOSE
        )
        if match:
            return {
        ...
        multimedia_index_query = parse_create_multimedia_index(query)
        if multimedia_index_query:
            return multimedia_index_query
        ...
        match = re.match(
            create_table_pattern, query, re.IGNORECASE | re.VERBOSE | re.DOTALL
        )
        if match:
            table_name = match.group(1)
            ...
            for col_def in column_defs:
                ...
                col_pattern = r"""
                    (\w+)\s+                                                # column name
                    ((?:VARCHAR\(\d+\)|POINT|POLYGON|LINESTRING|GEOMETRY|IMAGE|AUDIO|VIDEO|\w+))\s*  # data type 
                    (?:(KEY)\s*)?                                           # optional KEY
                ...
                if col_match:
                    ...
                    simple_pattern = r"(\w+)\s+((?:VARCHAR\(\d+\)|POINT|POLYGON|LINESTRING|GEOMETRY|IMAGE|AUDIO|VIDEO|\w+))"
...
def parse_create_multimedia_index(query):
    ...
    match = re.match(multimedia_index_pattern, query, re.IGNORECASE | re.VERBOSE)
    if match:
        return {
            "type": "CREATE_MULTIMEDIA_INDEX",
            "index_name": match.group(1),
            "table_name": match.group(2),
            "column_name": match.group(3),
            "media_type": match.group(4).lower(),
            "method": (
...
def parse_similarity_search(where_clause):
    """
    Parsea condiciones de similitud multimedia:
    - column SIMILAR TO "path" [LIMIT n]
    """
    # Búsqueda por similitud multimedia
    similarity_pattern = (
        r'(\w+)\s+SIMILAR\s+TO\s+[\'"]([^\'"]+)[\'"](?:\s+LIMIT\s+(\d+))?$'
    )
    match = re.match(similarity_pattern, where_clause, re.IGNORECASE)
    if match:
        column = match.group(1)
        query_file = match.group(2)
        limit = int(match.group(3)) if match.group(3) else 5

        return {
            "condition_type": "MULTIMEDIA_SIMILARITY",
            "column": column,
...
def parse_where_clause(where_clause):
    ...
    try:
        # Búsqueda por similitud multimedia
        similarity_condition = parse_similarity_search(where_clause)
        if similarity_condition:
            return similarity_condition
        ...
        ranked_pattern = (
            r'(\w+)\s+RANKED\s+BY\s+([\'"]?.+?[\'"]?)(?:\s+LIMIT\s+(\d+))?$'
        )
